...
package body UxAS.Comms.LMCP_Net_Client.Service.Route_Aggregator_Service.SPARK with SPARK_Mode is
            ...
            if (Is_Foud_Planning_State and Contains_Vehicles) then
               ...
               begin
                  ...
                  for Id_Opt  in First_Index(Task_Option_List) .. Last_Index(Task_Option_List) loop
                     declare
                        Option : constant My_TaskOption := Element(Task_Option_List,Id_Opt);
                        ...
                        Task_Option_Pair : constant AggregatorTaskOptionPair := AggregatorTaskOptionPair'(VehicleId      => Vehicle_ID,
                                                                                                          PrevTaskId     => 0,
                                                                                                          PrevTaskOption => 0,
                                                                                                          TaskId         => Get_TaskID   (Option),
                                                                                                          TaskOption     => Get_OptionID (Option));
                     ...
                     begin
                        ...
                        Set_EndLocation   (Route_Constraints, Get_StartLocation (Option));
                        Set_EndHeading    (Route_Constraints, Get_StartHeading  (Option));
   ...
   is
      ...
      Region_ID   : Int64 := Get_OperatingRegion (Route_Plan_Request);
      Vehicles_ID : Int64 := Get_VehicleID (Route_Plan_Request);
      Task_ID     : Int64 := Get_AssociatedTaskID (Route_Plan_Request);
   ...
   begin
      ...
      for K in First_Index(Get_RouteRequests(Route_Plan_Request)) .. Last_Index(Get_RouteRequests(Route_Plan_Request)) loop
         declare
            ...
            Route_Request : My_RouteConstraints := Element (Container => Get_RouteRequests(Route_Plan_Request),
                                                            Index  => K);
